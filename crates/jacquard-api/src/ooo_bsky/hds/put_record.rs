...
//
// Lexicon: ooo.bsky.hds.putRecord
//
...
pub struct PutRecordOutput<S: BosStr = DefaultStr> {
    pub cid: Cid<S>,
    pub uri: UriValue<S>,
    pub validation_status: PutRecordOutputValidationStatus<S>,
    #[serde(flatten, default, skip_serializing_if = "Option::is_none")]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}
...
/** Response marker for the `ooo.bsky.hds.putRecord` procedure.
...
/** Endpoint marker for the `ooo.bsky.hds.putRecord` procedure.

Path: `/xrpc/ooo.bsky.hds.putRecord`. The request payload type is `PutRecord<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
