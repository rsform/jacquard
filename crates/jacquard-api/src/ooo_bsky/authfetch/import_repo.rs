...
/** Response marker for the `ooo.bsky.authfetch.importRepo` procedure.
...
/** Endpoint marker for the `ooo.bsky.authfetch.importRepo` procedure.

Path: `/xrpc/ooo.bsky.authfetch.importRepo`. The request payload type is `ImportRepo`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
