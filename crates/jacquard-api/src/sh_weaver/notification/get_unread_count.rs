...
/** Response marker for the `sh.weaver.notification.getUnreadCount` query.
...
/** Endpoint marker for the `sh.weaver.notification.getUnreadCount` query.

Path: `/xrpc/sh.weaver.notification.getUnreadCount`. The request payload type is `GetUnreadCount`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
