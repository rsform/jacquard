...
/** Response marker for the `sh.weaver.notification.updateSeen` procedure.
...
/** Endpoint marker for the `sh.weaver.notification.updateSeen` procedure.

Path: `/xrpc/sh.weaver.notification.updateSeen`. The request payload type is `UpdateSeen<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
