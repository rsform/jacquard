...
pub struct GetFollowingOutput<S: BosStr = DefaultStr> {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cursor: Option<S>,
    pub following: Vec<ProfileViewBasic<S>>,
    pub subject: ProfileViewBasic<S>,
    #[serde(flatten, default, skip_serializing_if = "Option::is_none")]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

/** Response marker for the `sh.weaver.graph.getFollowing` query.
...
/** Endpoint marker for the `sh.weaver.graph.getFollowing` query.

Path: `/xrpc/sh.weaver.graph.getFollowing`. The request payload type is `GetFollowing<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
