...
/** Response marker for the `sh.weaver.graph.getSubscribers` query.
...
/** Endpoint marker for the `sh.weaver.graph.getSubscribers` query.

Path: `/xrpc/sh.weaver.graph.getSubscribers`. The request payload type is `GetSubscribers<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
    ...
    St::Notebook: get_subscribers_state::IsUnset,
{
    ...
    ) -> GetSubscribersBuilder<get_subscribers_state::SetNotebook<St>, S> {
