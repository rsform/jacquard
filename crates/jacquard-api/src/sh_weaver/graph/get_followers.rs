...
/** Response marker for the `sh.weaver.graph.getFollowers` query.
...
/** Endpoint marker for the `sh.weaver.graph.getFollowers` query.

Path: `/xrpc/sh.weaver.graph.getFollowers`. The request payload type is `GetFollowers<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
