...
pub enum GetListError {
    ...
    ListNotFound(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
...
/** Response marker for the `sh.weaver.graph.getList` query.
...
/** Endpoint marker for the `sh.weaver.graph.getList` query.

Path: `/xrpc/sh.weaver.graph.getList`. The request payload type is `GetList<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
