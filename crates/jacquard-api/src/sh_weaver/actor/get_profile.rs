...
pub struct GetProfileOutput<S: BosStr = DefaultStr> {
    #[serde(flatten)]
    pub value: ProfileDataView<S>,
    #[serde(flatten, default, skip_serializing_if = "Option::is_none")]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

/** Response marker for the `sh.weaver.actor.getProfile` query.
...
/** Endpoint marker for the `sh.weaver.actor.getProfile` query.

Path: `/xrpc/sh.weaver.actor.getProfile`. The request payload type is `GetProfile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
