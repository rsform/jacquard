...
/** Response marker for the `sh.weaver.notebook.getEntryNotebooks` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetEntryNotebooksOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `sh.weaver.notebook.getEntryNotebooks` query.

Path: `/xrpc/sh.weaver.notebook.getEntryNotebooks`. The request payload type is `GetEntryNotebooks<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
