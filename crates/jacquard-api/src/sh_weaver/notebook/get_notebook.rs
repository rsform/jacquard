...
/** Response marker for the `sh.weaver.notebook.getNotebook` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetNotebookOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `sh.weaver.notebook.getNotebook` query.

Path: `/xrpc/sh.weaver.notebook.getNotebook`. The request payload type is `GetNotebook<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
