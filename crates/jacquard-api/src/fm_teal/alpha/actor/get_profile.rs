...
/** Response marker for the `fm.teal.alpha.actor.getProfile` query.
...
/** Endpoint marker for the `fm.teal.alpha.actor.getProfile` query.

Path: `/xrpc/fm.teal.alpha.actor.getProfile`. The request payload type is `GetProfile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
