...
/** Response marker for the `fm.teal.alpha.stats.getTopReleases` query.
...
/** Endpoint marker for the `fm.teal.alpha.stats.getTopReleases` query.

Path: `/xrpc/fm.teal.alpha.stats.getTopReleases`. The request payload type is `GetTopReleases<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
