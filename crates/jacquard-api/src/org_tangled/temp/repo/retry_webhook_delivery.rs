// @generated by jacquard-lexicon. DO NOT EDIT.
//
// Lexicon: org.tangled.temp.repo.retryWebhookDelivery
//
// This file was automatically generated from Lexicon schemas.
// Any manual changes will be overwritten on the next regeneration.

#[allow(unused_imports)]
use alloc::collections::BTreeMap;

#[allow(unused_imports)]
use core::marker::PhantomData;
use jacquard_common::deps::smol_str::SmolStr;
use jacquard_common::types::string::Did;
use jacquard_common::types::value::Data;
use jacquard_common::{BosStr, CowStr, DefaultStr, FromStaticStr};
use jacquard_derive::{IntoStatic, open_union};
use serde::{Deserialize, Serialize};

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, IntoStatic)]
#[serde(
    rename_all = "camelCase",
    bound(deserialize = "S: Deserialize<'de> + BosStr")
)]
pub struct RetryWebhookDelivery<S: BosStr = DefaultStr> {
    ///UUID of the delivery to retry.
    pub delivery_id: S,
    ///DID of the repository as minted by the knot.
    pub repo_did: Did<S>,
    ///Webhook ID that owns the delivery.
    pub webhook_id: i64,
    #[serde(flatten, default, skip_serializing_if = "Option::is_none")]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

#[derive(
    Serialize, Deserialize, Debug, Clone, PartialEq, Eq, thiserror::Error, miette::Diagnostic,
)]
#[serde(tag = "error", content = "message")]
pub enum RetryWebhookDeliveryError {
    #[serde(rename = "WebhookNotFound")]
    WebhookNotFound(Option<SmolStr>),
    #[serde(rename = "DeliveryNotFound")]
    DeliveryNotFound(Option<SmolStr>),
    /// Catch-all for unknown error codes.
    #[serde(untagged)]
    Other {
        error: SmolStr,
        message: Option<SmolStr>,
    },
}

impl core::fmt::Display for RetryWebhookDeliveryError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::WebhookNotFound(msg) => {
                write!(f, "WebhookNotFound")?;
                if let Some(msg) = msg {
                    write!(f, ": {}", msg)?;
                }
                Ok(())
            }
            Self::DeliveryNotFound(msg) => {
                write!(f, "DeliveryNotFound")?;
                if let Some(msg) = msg {
                    write!(f, ": {}", msg)?;
                }
                Ok(())
            }
            Self::Other { error, message } => {
                write!(f, "{}", error)?;
                if let Some(msg) = message {
                    write!(f, ": {}", msg)?;
                }
                Ok(())
            }
        }
    }
}

/** Response marker for the `org.tangled.temp.repo.retryWebhookDelivery` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `()` for this endpoint.*/
pub struct RetryWebhookDeliveryResponse;
impl jacquard_common::xrpc::XrpcResp for RetryWebhookDeliveryResponse {
    const NSID: &'static str = "org.tangled.temp.repo.retryWebhookDelivery";
    const ENCODING: &'static str = "application/json";
    type Output<S: BosStr> = ();
    type Err = RetryWebhookDeliveryError;
}

impl<S: BosStr> jacquard_common::xrpc::XrpcRequest for RetryWebhookDelivery<S> {
    const NSID: &'static str = "org.tangled.temp.repo.retryWebhookDelivery";
    const METHOD: jacquard_common::xrpc::XrpcMethod =
        jacquard_common::xrpc::XrpcMethod::Procedure("application/json");
    type Response = RetryWebhookDeliveryResponse;
}

/** Endpoint marker for the `org.tangled.temp.repo.retryWebhookDelivery` procedure.

Path: `/xrpc/org.tangled.temp.repo.retryWebhookDelivery`. The request payload type is `RetryWebhookDelivery<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
pub struct RetryWebhookDeliveryRequest;
impl jacquard_common::xrpc::XrpcEndpoint for RetryWebhookDeliveryRequest {
    const PATH: &'static str = "/xrpc/org.tangled.temp.repo.retryWebhookDelivery";
    const METHOD: jacquard_common::xrpc::XrpcMethod =
        jacquard_common::xrpc::XrpcMethod::Procedure("application/json");
    type Request<S: BosStr> = RetryWebhookDelivery<S>;
    type Response = RetryWebhookDeliveryResponse;
}

pub mod retry_webhook_delivery_state {

    pub use crate::builder_types::{IsSet, IsUnset, Set, Unset};
    #[allow(unused)]
    use ::core::marker::PhantomData;
    mod sealed {
        pub trait Sealed {}
    }
    /// State trait tracking which required fields have been set
    pub trait State: sealed::Sealed {
        type DeliveryId;
        type RepoDid;
        type WebhookId;
    }
    /// Empty state - all required fields are unset
    pub struct Empty(());
    impl sealed::Sealed for Empty {}
    impl State for Empty {
        type DeliveryId = Unset;
        type RepoDid = Unset;
        type WebhookId = Unset;
    }
    ///State transition - sets the `delivery_id` field to Set
    pub struct SetDeliveryId<St: State = Empty>(PhantomData<fn() -> St>);
    impl<St: State> sealed::Sealed for SetDeliveryId<St> {}
    impl<St: State> State for SetDeliveryId<St> {
        type DeliveryId = Set<members::delivery_id>;
        type RepoDid = St::RepoDid;
        type WebhookId = St::WebhookId;
    }
    ///State transition - sets the `repo_did` field to Set
    pub struct SetRepoDid<St: State = Empty>(PhantomData<fn() -> St>);
    impl<St: State> sealed::Sealed for SetRepoDid<St> {}
    impl<St: State> State for SetRepoDid<St> {
        type DeliveryId = St::DeliveryId;
        type RepoDid = Set<members::repo_did>;
        type WebhookId = St::WebhookId;
    }
    ///State transition - sets the `webhook_id` field to Set
    pub struct SetWebhookId<St: State = Empty>(PhantomData<fn() -> St>);
    impl<St: State> sealed::Sealed for SetWebhookId<St> {}
    impl<St: State> State for SetWebhookId<St> {
        type DeliveryId = St::DeliveryId;
        type RepoDid = St::RepoDid;
        type WebhookId = Set<members::webhook_id>;
    }
    /// Marker types for field names
    #[allow(non_camel_case_types)]
    pub mod members {
        ///Marker type for the `delivery_id` field
        pub struct delivery_id(());
        ///Marker type for the `repo_did` field
        pub struct repo_did(());
        ///Marker type for the `webhook_id` field
        pub struct webhook_id(());
    }
}

/// Builder for constructing an instance of this type.
pub struct RetryWebhookDeliveryBuilder<
    St: retry_webhook_delivery_state::State,
    S: BosStr = DefaultStr,
> {
    _state: PhantomData<fn() -> St>,
    _fields: (Option<S>, Option<Did<S>>, Option<i64>),
    _type: PhantomData<fn() -> S>,
}

impl RetryWebhookDelivery<DefaultStr> {
    /// Create a new builder for this type, using the default string type (DefaultStr = SmolStr) if needed
    pub fn new() -> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::Empty, DefaultStr> {
        RetryWebhookDeliveryBuilder::new()
    }
}

impl<S: BosStr> RetryWebhookDelivery<S> {
    /// Create a new builder for this type
    pub fn builder() -> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::Empty, S> {
        RetryWebhookDeliveryBuilder::builder()
    }
}

impl RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::Empty, DefaultStr> {
    /// Create a new builder with all fields unset, using the default string type, if needed
    pub fn new() -> Self {
        RetryWebhookDeliveryBuilder {
            _state: PhantomData,
            _fields: (None, None, None),
            _type: PhantomData,
        }
    }
}

impl<S: BosStr> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::Empty, S> {
    /// Create a new builder with all fields unset
    pub fn builder() -> Self {
        RetryWebhookDeliveryBuilder {
            _state: PhantomData,
            _fields: (None, None, None),
            _type: PhantomData,
        }
    }
}

impl<St, S: BosStr> RetryWebhookDeliveryBuilder<St, S>
where
    St: retry_webhook_delivery_state::State,
    St::DeliveryId: retry_webhook_delivery_state::IsUnset,
{
    /// Set the `deliveryId` field (required)
    pub fn delivery_id(
        mut self,
        value: impl Into<S>,
    ) -> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::SetDeliveryId<St>, S> {
        self._fields.0 = Option::Some(value.into());
        RetryWebhookDeliveryBuilder {
            _state: PhantomData,
            _fields: self._fields,
            _type: PhantomData,
        }
    }
}

impl<St, S: BosStr> RetryWebhookDeliveryBuilder<St, S>
where
    St: retry_webhook_delivery_state::State,
    St::RepoDid: retry_webhook_delivery_state::IsUnset,
{
    /// Set the `repoDid` field (required)
    pub fn repo_did(
        mut self,
        value: impl Into<Did<S>>,
    ) -> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::SetRepoDid<St>, S> {
        self._fields.1 = Option::Some(value.into());
        RetryWebhookDeliveryBuilder {
            _state: PhantomData,
            _fields: self._fields,
            _type: PhantomData,
        }
    }
}

impl<St, S: BosStr> RetryWebhookDeliveryBuilder<St, S>
where
    St: retry_webhook_delivery_state::State,
    St::WebhookId: retry_webhook_delivery_state::IsUnset,
{
    /// Set the `webhookId` field (required)
    pub fn webhook_id(
        mut self,
        value: impl Into<i64>,
    ) -> RetryWebhookDeliveryBuilder<retry_webhook_delivery_state::SetWebhookId<St>, S> {
        self._fields.2 = Option::Some(value.into());
        RetryWebhookDeliveryBuilder {
            _state: PhantomData,
            _fields: self._fields,
            _type: PhantomData,
        }
    }
}

impl<St, S: BosStr> RetryWebhookDeliveryBuilder<St, S>
where
    St: retry_webhook_delivery_state::State,
    St::DeliveryId: retry_webhook_delivery_state::IsSet,
    St::RepoDid: retry_webhook_delivery_state::IsSet,
    St::WebhookId: retry_webhook_delivery_state::IsSet,
{
    /// Build the final struct.
    pub fn build(self) -> RetryWebhookDelivery<S> {
        RetryWebhookDelivery {
            delivery_id: self._fields.0.unwrap(),
            repo_did: self._fields.1.unwrap(),
            webhook_id: self._fields.2.unwrap(),
            extra_data: Default::default(),
        }
    }
    /// Build the final struct with custom extra_data.
    pub fn build_with_data(
        self,
        extra_data: BTreeMap<SmolStr, Data<S>>,
    ) -> RetryWebhookDelivery<S> {
        RetryWebhookDelivery {
            delivery_id: self._fields.0.unwrap(),
            repo_did: self._fields.1.unwrap(),
            webhook_id: self._fields.2.unwrap(),
            extra_data: Some(extra_data),
        }
    }
}
