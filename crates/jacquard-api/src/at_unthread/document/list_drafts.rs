...
/** Response marker for the `at.unthread.document.listDrafts` query.
...
/** Endpoint marker for the `at.unthread.document.listDrafts` query.

Path: `/xrpc/at.unthread.document.listDrafts`. The request payload type is `ListDrafts<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
