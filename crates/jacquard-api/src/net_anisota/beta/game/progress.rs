...
impl<'a> ::jacquard_lexicon::schema::LexiconSchema for Progress<'a> {
    ...
    fn validate(
        ...
        if self.previous_level < 1i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "previous_level",
                min: 1i64,
                actual: self.previous_level,
            });
        }
        if self.total_xp < 0i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "total_xp",
                min: 0i64,
                actual: self.total_xp,
            });
        }
        if self.xp_gained_since_last_save < 0i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "xp_gained_since_last_save",
                min: 0i64,
                actual: self.xp_gained_since_last_save,
            });
        }
        if self.xp_to_next_level < 0i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "xp_to_next_level",
                min: 0i64,
                actual: self.xp_to_next_level,
            });
        }
        Ok(())
    }
