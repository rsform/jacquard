...
impl<'a> ::jacquard_lexicon::schema::LexiconSchema for Inventory<'a> {
    ...
    fn validate(
        ...
        if self.item_id.len() > 100usize {
            return Err(::jacquard_lexicon::schema::ValidationError::MaxLength {
                field: "item_id",
                max: 100usize,
                actual: self.item_id.len(),
            });
        }
        if self.item_name.len() > 200usize {
            return Err(::jacquard_lexicon::schema::ValidationError::MaxLength {
                field: "item_name",
                max: 200usize,
                actual: self.item_name.len(),
            });
        }
        if self.item_type.len() > 50usize {
            return Err(::jacquard_lexicon::schema::ValidationError::MaxLength {
                field: "item_type",
                max: 50usize,
                actual: self.item_type.len(),
            });
        }
        if self.item_value < 0i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "item_value",
                min: 0i64,
                actual: self.item_value,
            });
        }
        if self.max_stack < 1i64 {
            return Err(::jacquard_lexicon::schema::ValidationError::Minimum {
                field: "max_stack",
                min: 1i64,
                actual: self.max_stack,
            });
        }
