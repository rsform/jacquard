...
pub enum CompleteUploadError {
    ...
    InvalidUploadId(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
    #[serde(rename = "InvalidDigest")]
    InvalidDigest(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
    ...
    CompletionFailed(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
...
pub struct PartInfo<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_part_info_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

/** Response marker for the `io.atcr.hold.completeUpload` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `CompleteUploadOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `io.atcr.hold.completeUpload` procedure.

Path: `/xrpc/io.atcr.hold.completeUpload`. The request payload type is `CompleteUpload<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
fn deserialize_part_info_extra_data<'de, S, D>(
    deserializer: D,
