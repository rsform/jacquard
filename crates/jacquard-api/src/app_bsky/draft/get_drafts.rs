...
/** Response marker for the `app.bsky.draft.getDrafts` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetDraftsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `app.bsky.draft.getDrafts` query.

Path: `/xrpc/app.bsky.draft.getDrafts`. The request payload type is `GetDrafts<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
