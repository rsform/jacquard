...
/** Response marker for the `games.gamesgamesgamesgames.acceptContribution` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `AcceptContributionOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `games.gamesgamesgamesgames.acceptContribution` procedure.

Path: `/xrpc/games.gamesgamesgamesgames.acceptContribution`. The request payload type is `AcceptContribution<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
