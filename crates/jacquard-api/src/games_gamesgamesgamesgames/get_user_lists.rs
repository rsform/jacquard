...
pub struct ListPreviewItem<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_list_preview_item_extra_data",
...
pub struct UserListView<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_user_list_view_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}
...
Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetUserListsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `games.gamesgamesgamesgames.getUserLists` query.

Path: `/xrpc/games.gamesgamesgamesgames.getUserLists`. The request payload type is `GetUserLists<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
fn deserialize_list_preview_item_extra_data<'de, S, D>(
    deserializer: D,
...
pub struct ListPreviewItemBuilder<St: list_preview_item_state::State, S: BosStr = DefaultStr> {
    ...
    St::Uri: list_preview_item_state::IsSet,
{
    ...
    pub fn build_with_data(self, extra_data: BTreeMap<SmolStr, Data<S>>) -> ListPreviewItem<S> {
        ListPreviewItem {
            media: self._fields.0,
            name: self._fields.1.unwrap(),
            slug: self._fields.2,
...
fn deserialize_user_list_view_extra_data<'de, S, D>(
    deserializer: D,
) -> Result<Option<BTreeMap<SmolStr, Data<S>>>, D::Error>
where
    S: BosStr + serde::Deserialize<'de>,
