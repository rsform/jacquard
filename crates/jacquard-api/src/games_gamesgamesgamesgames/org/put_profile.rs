...
/** Response marker for the `games.gamesgamesgamesgames.org.putProfile` procedure.
...
/** Endpoint marker for the `games.gamesgamesgamesgames.org.putProfile` procedure.

Path: `/xrpc/games.gamesgamesgamesgames.org.putProfile`. The request payload type is `PutProfile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
