...
pub struct SlugResult<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_slug_result_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

/** Response marker for the `games.gamesgamesgamesgames.searchSlugs` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `SearchSlugsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `games.gamesgamesgamesgames.searchSlugs` query.

Path: `/xrpc/games.gamesgamesgamesgames.searchSlugs`. The request payload type is `SearchSlugs<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
fn deserialize_slug_result_extra_data<'de, S, D>(
    deserializer: D,
