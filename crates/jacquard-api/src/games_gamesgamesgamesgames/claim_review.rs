...
pub struct ClaimReview<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_claim_review_extra_data",
...
fn deserialize_claim_review_extra_data<'de, S, D>(
    deserializer: D,
    ...
    St::Status: claim_review_state::IsSet,
{
    ...
    pub fn build_with_data(self, extra_data: BTreeMap<SmolStr, Data<S>>) -> ClaimReview<S> {
        ClaimReview {
            approved_games: self._fields.0,
            claim: self._fields.1.unwrap(),
            created_at: self._fields.2.unwrap(),
