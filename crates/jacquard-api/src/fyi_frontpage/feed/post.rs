...
pub struct UrlSubject<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_url_subject_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}
...
impl<S: BosStr> LexiconSchema for UrlSubject<S> {
    fn nsid() -> &'static str {
        "fyi.frontpage.feed.post"
    }
    fn def_name() -> &'static str {
...
fn deserialize_url_subject_extra_data<'de, S, D>(
    deserializer: D,
) -> Result<Option<BTreeMap<SmolStr, Data<S>>>, D::Error>
where
    S: BosStr + serde::Deserialize<'de>,
...
{
    /// Build the final struct.
    pub fn build(self) -> UrlSubject<S> {
        UrlSubject {
