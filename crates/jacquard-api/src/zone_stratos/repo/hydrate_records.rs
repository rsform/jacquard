...
/** Response marker for the `zone.stratos.repo.hydrateRecords` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `HydrateRecordsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `zone.stratos.repo.hydrateRecords` procedure.

Path: `/xrpc/zone.stratos.repo.hydrateRecords`. The request payload type is `HydrateRecords<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
pub struct HydrateRecordsBuilder<St: hydrate_records_state::State, S: BosStr = DefaultStr> {
