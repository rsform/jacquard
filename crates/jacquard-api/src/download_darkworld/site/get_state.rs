...
pub struct Output<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_output_extra_data",
...
/** Request marker for the `download.darkworld.site.getState` query.

This endpoint has no request parameters or input body; send this marker with `jacquard::Client`.*/

#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, IntoStatic, Copy)]
pub struct GetState;
/** Response marker for the `download.darkworld.site.getState` query.
...
/** Endpoint marker for the `download.darkworld.site.getState` query.

Path: `/xrpc/download.darkworld.site.getState`. The request payload type is `GetState`; use this marker with lower-level `XrpcEndpoint` APIs when you need endpoint metadata.*/
...
fn deserialize_output_extra_data<'de, S, D>(
    deserializer: D,
