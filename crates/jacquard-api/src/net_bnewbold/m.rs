...
pub struct M<S: Bos<str> + AsRef<str> = DefaultStr> {
    pub common_name: S,
...
pub struct MGetRecordOutput<S: Bos<str> + AsRef<str> = DefaultStr> {
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl XrpcResp for MRecord {
    ...
    type Output<S: Bos<str> + AsRef<str>> = MGetRecordOutput<S>;
    type Err = RecordError;
}

impl<S: Bos<str> + AsRef<str>> From<MGetRecordOutput<S>> for M<S> {
    fn from(output: MGetRecordOutput<S>) -> Self {
        output.value
    }
}

impl<S: Bos<str> + AsRef<str>> Collection for M<S> {
    const NSID: &'static str = "net.bnewbold.m";
    type Record = MRecord;
}
...
impl<S: Bos<str> + AsRef<str>> LexiconSchema for M<S> {
    fn nsid() -> &'static str {
        "net.bnewbold.m"
    }
    ...
    S::CommonName: m_state::IsSet,
{
    ...
    pub fn build_with_data(self, extra_data: BTreeMap<SmolStr, Data<'a>>) -> M<'a> {
        M {
            common_name: self._fields.0.unwrap(),
            edible: self._fields.1,
            species: self._fields.2,
