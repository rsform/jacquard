...
/** Response marker for the `social.clippr.actor.getProfile` query.
...
/** Endpoint marker for the `social.clippr.actor.getProfile` query.

Path: `/xrpc/social.clippr.actor.getProfile`. The request payload type is `GetProfile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
