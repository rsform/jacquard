...
/** Response marker for the `social.clippr.actor.putPreferences` procedure.
...
/** Endpoint marker for the `social.clippr.actor.putPreferences` procedure.

Path: `/xrpc/social.clippr.actor.putPreferences`. The request payload type is `PutPreferences<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
