...
/** Response marker for the `org.atsui.Clip` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `ClipOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `org.atsui.Clip` procedure.

Path: `/xrpc/org.atsui.Clip`. The request payload type is `Clip<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
