...
/** Response marker for the `org.atsui.Link` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `LinkOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `org.atsui.Link` procedure.

Path: `/xrpc/org.atsui.Link`. The request payload type is `Link<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
