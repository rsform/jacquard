...
/** Response marker for the `org.atsui.Caption` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `CaptionOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `org.atsui.Caption` procedure.

Path: `/xrpc/org.atsui.Caption`. The request payload type is `Caption<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
