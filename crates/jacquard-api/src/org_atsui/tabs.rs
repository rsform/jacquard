...
pub struct Tab<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_tab_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}

/** Response marker for the `org.atsui.Tabs` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `TabsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `org.atsui.Tabs` procedure.

Path: `/xrpc/org.atsui.Tabs`. The request payload type is `Tabs<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
fn deserialize_tab_extra_data<'de, S, D>(
    deserializer: D,
