...
/** Response marker for the `org.passingreads.book.getLibrary` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetLibraryOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `org.passingreads.book.getLibrary` query.

Path: `/xrpc/org.passingreads.book.getLibrary`. The request payload type is `GetLibrary<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
