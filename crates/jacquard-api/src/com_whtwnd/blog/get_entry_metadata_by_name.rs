...
/** Response marker for the `com.whtwnd.blog.getEntryMetadataByName` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetEntryMetadataByNameOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `com.whtwnd.blog.getEntryMetadataByName` query.

Path: `/xrpc/com.whtwnd.blog.getEntryMetadataByName`. The request payload type is `GetEntryMetadataByName<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
