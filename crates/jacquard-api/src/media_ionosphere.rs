...
//! Generated bindings for the `media.ionosphere` Lexicon namespace/module.
pub mod group;
pub mod log;
pub mod programme;
pub mod service;
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum EntityType<S: BosStr = DefaultStr> {
    Organization,
    Person,
    Other(S),
}

impl<S: BosStr> EntityType<S> {
    pub fn as_str(&self) -> &str {
        match self {
...
impl<S: BosStr> core::fmt::Display for EntityType<S> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

impl<S: BosStr> AsRef<str> for EntityType<S> {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl<S: BosStr> Serialize for EntityType<S> {
    fn serialize<Ser>(&self, serializer: Ser) -> Result<Ser::Ok, Ser::Error>
    where
        Ser: serde::Serializer,
    {
...
impl<'de, S: Deserialize<'de> + BosStr> Deserialize<'de> for EntityType<S> {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
...
impl<S: BosStr + Default> Default for EntityType<S> {
    fn default() -> Self {
        Self::Other(Default::default())
    }
}

impl<S: BosStr> jacquard_common::IntoStatic for EntityType<S>
where
    S: BosStr + jacquard_common::IntoStatic,
    S::Output: BosStr,
{
    type Output = EntityType<S::Output>;
    fn into_static(self) -> Self::Output {
        match self {
            EntityType::Organization => EntityType::Organization,
            EntityType::Person => EntityType::Person,
            EntityType::Other(v) => EntityType::Other(v.into_static()),
        }
    }
}
