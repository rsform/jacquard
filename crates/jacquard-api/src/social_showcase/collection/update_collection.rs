...
/** Response marker for the `social.showcase.collection.updateCollection` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `UpdateCollectionOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `social.showcase.collection.updateCollection` procedure.

Path: `/xrpc/social.showcase.collection.updateCollection`. The request payload type is `UpdateCollection<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
pub struct UpdateCollectionBuilder<St: update_collection_state::State, S: BosStr = DefaultStr> {
