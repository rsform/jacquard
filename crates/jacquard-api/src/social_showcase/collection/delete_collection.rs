...
/** Response marker for the `social.showcase.collection.deleteCollection` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `DeleteCollectionOutput` for this endpoint.*/
...
/** Endpoint marker for the `social.showcase.collection.deleteCollection` procedure.

Path: `/xrpc/social.showcase.collection.deleteCollection`. The request payload type is `DeleteCollection<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
