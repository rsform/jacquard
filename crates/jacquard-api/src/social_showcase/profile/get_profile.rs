...
//
// Lexicon: social.showcase.profile.getProfile
//
...
/** Response marker for the `social.showcase.profile.getProfile` query.
...
/** Endpoint marker for the `social.showcase.profile.getProfile` query.

Path: `/xrpc/social.showcase.profile.getProfile`. The request payload type is `GetProfile<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
