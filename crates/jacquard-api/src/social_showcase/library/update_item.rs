...
/** Response marker for the `social.showcase.library.updateItem` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `UpdateItemOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `social.showcase.library.updateItem` procedure.

Path: `/xrpc/social.showcase.library.updateItem`. The request payload type is `UpdateItem<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
