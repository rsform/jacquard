...
pub enum UpdatePostError {
    ...
    PostNotPending(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
    #[serde(rename = "NoFieldsProvided")]
    NoFieldsProvided(#[serde(skip_serializing_if = "Option::is_none")] Option<SmolStr>),
...
/** Response marker for the `app.chronosky.schedule.updatePost` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `UpdatePostOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `app.chronosky.schedule.updatePost` procedure.

Path: `/xrpc/app.chronosky.schedule.updatePost`. The request payload type is `UpdatePost<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
