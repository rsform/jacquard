...
/** Response marker for the `uk.skyblur.post.store` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `StoreOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `uk.skyblur.post.store` procedure.

Path: `/xrpc/uk.skyblur.post.store`. The request payload type is `Store<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
