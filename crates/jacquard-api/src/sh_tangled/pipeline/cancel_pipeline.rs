...
/** Response marker for the `sh.tangled.pipeline.cancelPipeline` procedure.
...
/** Endpoint marker for the `sh.tangled.pipeline.cancelPipeline` procedure.

Path: `/xrpc/sh.tangled.pipeline.cancelPipeline`. The request payload type is `CancelPipeline<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
