...
//
// Lexicon: sh.tangled.publicKey.listKeys
//
...
use crate::sh_tangled::publicKey::list_keys;
...
pub enum ListKeysOrder<S: BosStr = DefaultStr> {
    Asc,
    Desc,
    Other(S),
}

impl<S: BosStr> ListKeysOrder<S> {
...
impl<S: BosStr> core::fmt::Display for ListKeysOrder<S> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

impl<S: BosStr> AsRef<str> for ListKeysOrder<S> {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl<S: BosStr> Serialize for ListKeysOrder<S> {
...
impl<'de, S: Deserialize<'de> + BosStr> Deserialize<'de> for ListKeysOrder<S> {
...
impl<S: BosStr + Default> Default for ListKeysOrder<S> {
    fn default() -> Self {
        Self::Other(Default::default())
    }
}

impl<S: BosStr> jacquard_common::IntoStatic for ListKeysOrder<S>
where
    S: BosStr + jacquard_common::IntoStatic,
    S::Output: BosStr,
{
    type Output = ListKeysOrder<S::Output>;
    fn into_static(self) -> Self::Output {
        match self {
            ListKeysOrder::Asc => ListKeysOrder::Asc,
            ListKeysOrder::Desc => ListKeysOrder::Desc,
            ListKeysOrder::Other(v) => ListKeysOrder::Other(v.into_static()),
        }
    }
}
...
pub struct ListKeys<S: BosStr = DefaultStr> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub order: Option<ListKeysOrder<S>>,
    pub subject: Did<S>,
}
...
pub struct ListKeysOutput<S: BosStr = DefaultStr> {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cursor: Option<S>,
    pub items: Vec<list_keys::ListItem<S>>,
...
impl<S: BosStr> LexiconSchema for ListItem<S> {
    ...
    fn lexicon_doc() -> LexiconDoc<'static> {
        lexicon_doc_sh_tangled_publicKey_listKeys()
    }
...
/** Response marker for the `sh.tangled.publicKey.listKeys` query.
...
impl jacquard_common::xrpc::XrpcResp for ListKeysResponse {
    const NSID: &'static str = "sh.tangled.publicKey.listKeys";
...
/** Endpoint marker for the `sh.tangled.publicKey.listKeys` query.

Path: `/xrpc/sh.tangled.publicKey.listKeys`. The request payload type is `ListKeys<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
pub struct ListKeysRequest;
impl jacquard_common::xrpc::XrpcEndpoint for ListKeysRequest {
    const PATH: &'static str = "/xrpc/sh.tangled.publicKey.listKeys";
    const METHOD: jacquard_common::xrpc::XrpcMethod = jacquard_common::xrpc::XrpcMethod::Query;
...
fn lexicon_doc_sh_tangled_publicKey_listKeys() -> LexiconDoc<'static> {
    #[allow(unused_imports)]
    ...
    LexiconDoc {
        ...
        id: CowStr::new_static("sh.tangled.publicKey.listKeys"),
        defs: {
...
fn _default_order<S: jacquard_common::BosStr + jacquard_common::FromStaticStr>() -> Option<
    ListKeysOrder<S>,
> {
    Some(<ListKeysOrder<S>>::from_value(S::from_static("desc")))
}
...
pub struct ListKeysBuilder<St: list_keys_state::State, S: BosStr = DefaultStr> {
    _state: PhantomData<fn() -> St>,
    _fields: (Option<S>, Option<i64>, Option<ListKeysOrder<S>>, Option<Did<S>>),
    _type: PhantomData<fn() -> S>,
}
...
impl<St: list_keys_state::State, S: BosStr> ListKeysBuilder<St, S> {
    ...
    pub fn order(mut self, value: impl Into<Option<ListKeysOrder<S>>>) -> Self {
        self._fields.2 = value.into();
        self
    }
    /// Set the `order` field to an Option value (optional)
    pub fn maybe_order(mut self, value: Option<ListKeysOrder<S>>) -> Self {
...
where
    ...
    St::Subject: list_keys_state::IsUnset,
{
    /// Set the `subject` field (required)
    pub fn subject(
        mut self,
        value: impl Into<Did<S>>,
    ) -> ListKeysBuilder<list_keys_state::SetSubject<St>, S> {
...
where
    St: list_keys_state::State,
    St::Subject: list_keys_state::IsSet,
{
    /// Build the final struct.
