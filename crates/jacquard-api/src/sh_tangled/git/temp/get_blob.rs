...
/** Response marker for the `sh.tangled.git.temp.getBlob` query.
...
/** Endpoint marker for the `sh.tangled.git.temp.getBlob` query.

Path: `/xrpc/sh.tangled.git.temp.getBlob`. The request payload type is `GetBlob<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
    ...
    St::Path: get_blob_state::IsUnset,
{
    ...
    pub fn path(mut self, value: impl Into<S>) -> GetBlobBuilder<get_blob_state::SetPath<St>, S> {
