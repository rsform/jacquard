...
/** Response marker for the `sh.tangled.git.temp.analyzeMerge` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `AnalyzeMergeOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `sh.tangled.git.temp.analyzeMerge` query.

Path: `/xrpc/sh.tangled.git.temp.analyzeMerge`. The request payload type is `AnalyzeMerge<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
