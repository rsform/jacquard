...
pub struct IssueListItem<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_issue_list_item_extra_data",
...
/** Response marker for the `sh.tangled.repo.listIssues` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `ListIssuesOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `sh.tangled.repo.listIssues` query.

Path: `/xrpc/sh.tangled.repo.listIssues`. The request payload type is `ListIssues<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
fn deserialize_issue_list_item_extra_data<'de, S, D>(
    deserializer: D,
