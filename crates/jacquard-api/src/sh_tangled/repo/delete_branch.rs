...
/** Response marker for the `sh.tangled.repo.deleteBranch` procedure.
...
/** Endpoint marker for the `sh.tangled.repo.deleteBranch` procedure.

Path: `/xrpc/sh.tangled.repo.deleteBranch`. The request payload type is `DeleteBranch<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
