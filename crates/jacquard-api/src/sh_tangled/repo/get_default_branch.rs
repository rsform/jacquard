...
/** Response marker for the `sh.tangled.repo.getDefaultBranch` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetDefaultBranchOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `sh.tangled.repo.getDefaultBranch` query.

Path: `/xrpc/sh.tangled.repo.getDefaultBranch`. The request payload type is `GetDefaultBranch<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
...
pub struct GetDefaultBranchBuilder<St: get_default_branch_state::State, S: BosStr = DefaultStr> {
...
impl GetDefaultBranch<DefaultStr> {
    ...
    pub fn new() -> GetDefaultBranchBuilder<get_default_branch_state::Empty, DefaultStr> {
