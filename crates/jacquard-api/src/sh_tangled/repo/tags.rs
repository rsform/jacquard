...
/** Response marker for the `sh.tangled.repo.tags` query.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `TagsOutput` for this endpoint.*/
...
/** Endpoint marker for the `sh.tangled.repo.tags` query.

Path: `/xrpc/sh.tangled.repo.tags`. The request payload type is `Tags<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
    ...
    St::Repo: tags_state::IsUnset,
{
    ...
    pub fn repo(mut self, value: impl Into<S>) -> TagsBuilder<tags_state::SetRepo<St>, S> {
