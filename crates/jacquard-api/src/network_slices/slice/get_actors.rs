...
/** Response marker for the `network.slices.slice.getActors` procedure.

Implements `jacquard_common::xrpc::XrpcResp`; successful bodies decode as `Self::Output<S>`, which is `GetActorsOutput<S>` for this endpoint.*/
...
/** Endpoint marker for the `network.slices.slice.getActors` procedure.

Path: `/xrpc/network.slices.slice.getActors`. The request payload type is `GetActors<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
