...
pub enum JobStatusStatus<S: BosStr = DefaultStr> {
    Pending,
    Running,
    Completed,
    Failed,
...
impl<S: BosStr> JobStatusStatus<S> {
...
impl<S: BosStr> core::fmt::Display for JobStatusStatus<S> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

impl<S: BosStr> AsRef<str> for JobStatusStatus<S> {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl<S: BosStr> Serialize for JobStatusStatus<S> {
...
impl<'de, S: Deserialize<'de> + BosStr> Deserialize<'de> for JobStatusStatus<S> {
...
impl<S: BosStr + Default> Default for JobStatusStatus<S> {
    fn default() -> Self {
        Self::Other(Default::default())
    }
}

impl<S: BosStr> jacquard_common::IntoStatic for JobStatusStatus<S>
where
    S: BosStr + jacquard_common::IntoStatic,
    S::Output: BosStr,
{
    type Output = JobStatusStatus<S::Output>;
    fn into_static(self) -> Self::Output {
        match self {
            JobStatusStatus::Pending => JobStatusStatus::Pending,
            JobStatusStatus::Running => JobStatusStatus::Running,
            JobStatusStatus::Completed => JobStatusStatus::Completed,
            JobStatusStatus::Failed => JobStatusStatus::Failed,
            JobStatusStatus::Other(v) => JobStatusStatus::Other(v.into_static()),
        }
    }
}
...
pub struct SyncJobResult<S: BosStr = DefaultStr> {
    ...
    #[serde(
        flatten,
        default,
        deserialize_with = "deserialize_sync_job_result_extra_data",
        skip_serializing_if = "Option::is_none"
    )]
    pub extra_data: Option<BTreeMap<SmolStr, Data<S>>>,
}
    ...
    St::Status: job_status_state::IsUnset,
{
    ...
    pub fn status(
        mut self,
        value: impl Into<JobStatusStatus<S>>,
...
fn deserialize_sync_job_result_extra_data<'de, S, D>(
    deserializer: D,
) -> Result<Option<BTreeMap<SmolStr, Data<S>>>, D::Error>
where
    S: BosStr + serde::Deserialize<'de>,
