...
/// Connect with arbitrary upgrade-request headers plus subprotocols.
///
/// Headers ride the HTTP upgrade request exactly as on a normal HTTP
/// client; `Sec-WebSocket-Protocol` is set from `protocols` in addition
/// to any caller-provided headers.
